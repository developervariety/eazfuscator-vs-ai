...
namespace eazdevirt
{
	...
	public class PositionTranslator : IPositionTranslator
	{
		...
		public virtual Int64 ToPosition(String str, Int32 cryptoKey)
		{
			...
			Int64 result = _streamType is CryptoStreamDefModern
				? ReadModernInt64(binaryReader)
				: binaryReader.ReadInt64();
			memoryStream.Dispose();
			return result;
		}

		private static Int64 ReadModernInt64(BinaryReader reader)
		{
			var b = reader.ReadBytes(8);
			if (b.Length != 8)
				throw new EndOfStreamException();

			UInt32 lo = ((UInt32)b[7] << 8)
			            | ((UInt32)b[2] << 24)
			            | b[0]
			            | ((UInt32)b[1] << 16);
			UInt32 hi = ((UInt32)b[5] << 24)
			            | ((UInt32)b[6] << 16)
			            | b[4]
			            | ((UInt32)b[3] << 8);
