...
namespace eazdevirt
{
	...
	public class VirtualMachineType
	{
		...
		private void Initialize()
		{
			...
			// Each of these depends on a specific IL shape from 2018-era
			// Eazfuscator. On newer builds any one of them may miss — treat
			// each as a soft failure and log, so we still get VType+Type
			// which is the majority of the useful information.
			TrySoft(InitializeArgumentsField, "arguments field");
			TrySoft(InitializeLocalsField,    "locals field");
			TrySoft(InitializeTypeFields,     "type fields");

			// Ensure TypeFields is always at least an empty dict so callers
			// can iterate without null-checks.
			if (this.TypeFields == null)
				this.TypeFields = new Dictionary<FieldDef, ITypeDefOrRef>();
		}

		private void TrySoft(Action step, string label)
		{
			try { step(); }
			catch (Exception ex)
			{
				this.Logger.Warning(this, "Skipped {0}: {1}", label, ex.Message);
			}
		}
